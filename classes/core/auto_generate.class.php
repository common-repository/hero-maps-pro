			...
			foreach($menu_object as $item){
				...
				if(isset($item['viewpath']) && $item['viewpath'] != ''){
					...
					if(!is_dir($dir) && $item['type'] != 'button'){
						...
						fwrite($handle, '<script type="text/javascript" src="<?php echo htmlspecialchars($_GET[\'v\'], ENT_QUOTES, \'UTF-8\'); ?>js/view.core.js" data-cfasync="false"></script>' ."\n");
						...
						foreach($item['views'] as $view){
							foreach($view['submenu'] as $sub){
								...
								if(!file_exists(realpath($dir) .'/'. $sub['view'] .'.view.php')){
									...
									fwrite($handle, '<script type="text/javascript" src="<?php echo htmlspecialchars($_GET[\'vp\'], ENT_QUOTES, \'UTF-8\'); ?>js/'. $sub['view'] .'.view.js" data-cfasync="false"></script>' ."\n");
									fclose($handle);
								}
							}
						}
