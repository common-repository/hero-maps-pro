		...
		public function update_map_object(){
			...
			$wpdb->query("
				UPDATE `". $wpdb->prefix ."hmapspro_maps` SET
					`name` = '". sanitize_text_field($map_object['map_setup']['map_name']) ."',
					`responsive` = ". $map_object['map_setup']['responsive'] .",
					`width` = ". intval($map_object['map_setup']['map_width']) .",
					`height` = ". intval($map_object['map_setup']['map_height']) .",
					...
					`marker_drop_delay` = ". intval($map_object['map_advanced']['marker_drop_delay']) .",
					`marker_animation` = '". $map_object['map_advanced']['marker_animation'] ."',
					`marker_animation_timer` = ". intval($map_object['map_advanced']['marker_animation_timer']) .",
					...
					`callback_method` = '". sanitize_text_field($map_object['map_developers']['callback_method']) ."',
					`css_class` = '". sanitize_text_field($map_object['map_developers']['css_class']) ."'
				WHERE
					...
					if($marker['new'] == 'true' && $marker['deleted'] == 'false'){ //add new marker
						...
						$result = $wpdb->query("
							...
							VALUES (
								...
								'". $marker['latlng'] ."',
								'". sanitize_text_field($marker['title']) ."',
								". $info_window_show .",
								'". sanitize_text_field($marker['info_window_content']) ."',
								". $link_show .",
								'". sanitize_text_field($marker['link_title']) ."',
								'". sanitize_text_field($marker['link']) ."',
								'". $marker['link_colour'] ."',
								'". $marker['link_target'] ."',
								'". sanitize_text_field($marker['custom_param']) ."'
							);
						");
						...
						$wpdb->query("
							...
							SET `deleted` = 1
							WHERE `map_marker_id` = ". intval($marker['map_marker_id']) .";
						");
						...
						$wpdb->query("
							UPDATE `". $wpdb->prefix ."hmapspro_map_markers` SET
								...
								`info_window_content` = '". sanitize_text_field($marker['info_window_content']) ."',
								`link_show` = ". $link_show .",
								`link_title` = '". sanitize_text_field($marker['link_title']) ."',
								`link` = '". sanitize_text_field($marker['link']) ."',
								`link_colour` = '". $marker['link_colour'] ."',
								`link_target` = '". $marker['link_target'] ."',
								`custom_param` = '". sanitize_text_field($marker['custom_param']) ."'
							WHERE
